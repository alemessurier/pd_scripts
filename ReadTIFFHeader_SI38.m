...
expr = '[^\n]*dir_processed=[^\n]*';
    ...
    eval(dp_string{:});
expression='numAvgFramesSave[^\n]*';
matchStr = regexp(str,expression,'match','dotexceptnewline'); 
tmp=eval(matchStr{:})% this returns whole line
tempindex = strfind(matchStr{1,1},'=');    % index of equal sign
Metadata.acqNumAveragedFrames = str2double(matchStr{1,1}(tempindex+1:end));     

expression = sprintf('frameRate.+\n');
...
expression = sprintf('numberOfChannelsSave.+\n');
matchStr = regexp(str,expression,'match','dotexceptnewline');    % this returns whole line
tempindex = strfind(matchStr{1,1},'=');    % index of equal sign
