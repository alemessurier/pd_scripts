...
idx_cell=logical(iscell(:,1));
F_include=F_np_corr(idx_cell,:);

% filter data w/moving median filter
filtF=zeros(size(F_include));
for i=1:size(filtF,1)
    cellName=['cell',num2str(i)];
    [ filtTrace.(cellName) ] = slidingAvg_rawF( F_include(i,:),5,'median' );
    filtF(i,:)=filtTrace.(cellName);
    rawF.(cellName)=F_include(i,:);
...
spacing=.5;
numCells=10;
plot_deltaF( deltaF,samp_rate,100,numCells,0.75 )
%%
...
%     thispath=filepaths(K,:);
%     idx_slash=strfind(thispath,'\');
%     thispath(idx_slash)='/';
%     numframes=read_numFrames_tiff(thispath);
     framesInAcq(K)=600;
end
...
plot_deltaF( mean_dF,samp_rate,10,20,0.75 )
