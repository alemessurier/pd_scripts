function US_summaryPlots_YiData(dF_byDS,stimFrame,dF_byPressure,DCsweep,...
    pressSweep,DC_labels,pressure_labels,samp_rate,ROI_masks,poststim)
...
%     subplot(1,2,1)
    allResponses_DC=cellfun(@(x)mean(dF_byDS.(x).(fns{dc}),2)',cellNames,'un',0);
...
%     subplot(1,2,2)
%     time=((1:length(meanResponse_P{dc}))-stimFrame)/samp_rate;
...
%     subplot(1,2,1)
    allResponses_DC=cellfun(@(x)median(dF_byDS.(x).(fns{dc}),2)',cellNames,'un',0);
%     allResponses=cellfun(@(x)(x-mean(x(1:stimFrame),2))/std(x(1:stimFrame)),allResponses,'un',0)
    allResponses_DC=cat(1,allResponses_DC{:});
    imagesc(allResponses_DC)
    colormap gray
...
prestim=(stimFrame-8):stimFrame;
% poststim=stimFrame:(stimFrame+60);
meansDC50=cellfun(@(x)mean(dF_byDS.(x).DC50,2)',cellNames,'un',0);
...
tmp=gca;
tmp.XTick=(((0:stimFrame:numFrames))/samp_rate)*samp_rate;
tmp.XTickLabel=((0:stimFrame:numFrames)-stimFrame)/samp_rate;
vline(stimFrame);
...
prestim=(stimFrame-9):stimFrame;
% poststim=stimFrame:(stimFrame+60);
mediansDC50=cellfun(@(x)median(dF_byDS.(x).DC50,2)',cellNames,'un',0);
...
plot_US_byROI_params( samp_rate,stimFrame,DCsweep,dF_byDS,pressSweep,dF_byPressure,[],pressure_labels,DC_labels)
