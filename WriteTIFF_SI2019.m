function WriteTIFF_SI2019( MovieData, Metadata, TIFF_Filename )
...
for (frame=2:size(MovieData,3))         % number of frames
    ...
    tagstruct.ImageWidth      = size(MovieData,2);
    tagstruct.Photometric     = Tiff.Photometric.MinIsBlack;
    tagstruct.BitsPerSample   = Metadata.InfoImage(1).BitsPerSample;
    tagstruct.SamplesPerPixel = Metadata.InfoImage(1).SamplesPerPixel;
    tagstruct.RowsPerStrip    = Metadata.InfoImage(1).RowsPerStrip;
    tagstruct.PlanarConfiguration = Metadata.InfoImage(1).PlanarConfiguration;
    tagstruct.Software        = 'MATLAB';
