function [ rawTimeSeries] = get_fluoTimeSeries_2020( varargin )
...
rawTimeSeries=cell(size(ROI_positions,3),1);
for r=1:size(ROI_positions,3)

    roi_mask=logical(ROI_positions(:,:,r));
    roi_mask_frames=repmat(roi_mask,1,1,size(rawStack,3));
    roiFluorTest=rawStack(roi_mask_frames);
    roiFluorByFrame=reshape(roiFluorTest,sum(sum(roi_mask)),length(roiFluorTest)/sum(sum(roi_mask)));
    roiFluor=mean(roiFluorByFrame,1);   
