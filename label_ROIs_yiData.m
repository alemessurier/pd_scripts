function [ROI_positions,neuropil_mask]=label_ROIs_yiData(dir_ROIs,ROI_positions,ring,gap)
...
if isempty(ROI_positions)
    ROI_positions=zeros(256,256);
end
    
for K=1:length(stackNames);  % loop every 5 movie files
    imPath=strcat(dir_ROIs,stackNames{K});
    [im, ~] = LoadTIFF_SI5(imPath);
 ROIs_new=define_ROI_masks_yiData(im,ROI_positions); % ROI(:,:,1)=zeros(512,512) in function
%     ROIs_new=define_ROI_masks(im,ROI_positions);
    if isempty(ROI_positions)&& K==1
...
% save([dir_ROIs,'\ROI_positions.mat'],'ROI_positions')
...
[neuropil_mask]=HW_makeNpMask_yiData(ROI_positions,ring,gap); 
