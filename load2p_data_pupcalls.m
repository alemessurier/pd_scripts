function [F_calls,F_byToneRep,ops,inds_ex,Fcell]=load2p_data_pupcalls(path_2p)
...
% remove files that are the wrong length
lengths=arrayfun(@(x)size(x.FROI,2),F_byFile);
exc=lengths<1000;
F_byFile=F_byFile(~exc);
fnames=fnames(~exc);

idx_calls1=cellfun(@(x)contains(x,'61','IgnoreCase',true),fnames);
