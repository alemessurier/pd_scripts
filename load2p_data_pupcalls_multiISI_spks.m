function [F_calls_2sec,F_calls_5sec,F_calls_10sec,F_byToneRep_3sec,F_byToneRep_5sec,ops,inds_ex,Fcell]=load2p_data_pupcalls_multiISI_spks(path_2p)
%% 20200824 loads in data processed by suite2p (Fall.mat), chunks fluorescence traces by file

load([path_2p,'Fall.mat'],'F','Fneu','ops','iscell','spks');
...
spks_include=spks(~inds_ex,:);


%% break fluorescence time series, spikes, and registration shifts into files
F_byFile=struct();
for f=1:length(fnames)
    ...
    F_byFile(f).spikes=spks_include(:,frame_start:frame_end);
end
