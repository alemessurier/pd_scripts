T=readtable('/Users/aml717/Desktop/patch/patch_meta.xlsx');
...
for c=1:length(metadata)
    ...
    try
        ...
        if exist('raster_all','var')
            ...
            [~,si]=abfload(abfs_tones{1}); % read sampling interval from first abf
            sampRateHz=1/(si*1e-6); % si is in microseconds
            [spike_raster,spikesByStim,threshold,raster_all,spike_times]=plot_toneTuning_patch(abfs_tones,invert,freq_order,threshold);
            ...
            data(c).tones.freq_order=freq_order;
            data(c).tones.sampRateHz=sampRateHz;
            save([fnames_tone{1},'_tones.mat'],'spike_raster','raster_all','spikesByStim','threshold','spike_times')
        end
        ...
        switch tmp
            ...
            case 0
                [spike_raster,spikesByStim,threshold,raster_all,spike_times]=plot_toneTuning_patch(abfs_tones,invert,freq_order,[]);
...
save([save_path_struct,'allpatch_reduced.mat'],'metadata','data')