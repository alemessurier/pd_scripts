function [allDates_calls,allDates_tones,MultiDayROIs_calls,MultiDayROIs_tones]= pupCall_multidayROIs_load(matchFilePath,paths_2p,save_path)
...
[allDates_calls,allDates_tones]=load_multiDay_data(paths_2p);
...
for d=1:length(paths_2p)
    idx_include=strfind(paths_2p{d},'reduced/');
    matchTest=paths_2p{d}(idx_include:end);
    tmpmatch=cellfun(@(x)contains(x,matchTest),matchDates);
...
MultiDayROIs_calls=make_multiDayROIs_struct_calls(allDates_calls,mapping);
MultiDayROIs_tones=make_multiDayROIs_struct_tones(allDates_tones,mapping);
...
save([save_path,'MultiDayROIs.mat'],'allDates_calls','allDates_tones','MultiDayROIs_calls','MultiDayROIs_tones')
%%
    function [allDates_calls,allDates_tones]=load_multiDay_data(paths_2p)
        %load data
        ...
        for d=1:length(paths_2p)
            ...
            allDates_calls(d).meanDF_total_calls=mean(deltaF_calls,2);
            allDates_tones(d).meanDF_total_tones=mean(deltaF_tones,2);
            allDates_calls(d).meanByTrial_early = meanByTrial_early;
            allDates_calls(d).meanByTrial_late = meanByTrial_late;
            allDates_calls(d).h_calls= h_calls;
            allDates_calls(d).responsiveTrials=responsiveTrials_byCell;
            allDates_calls(d).dFByTrial=dFByTrial;
            allDates_tones(d).dFByTone=dFByTone;
            allDates_tones(d).toneTuning=TUNING;
            allDates_tones(d).tones=TUNING(1).tones;
            allDates_tones(d).meanRTones=cat(1,TUNING(:).mean);
            allDates_tones(d).h_tone=h_pass;
        end
    end
        ...
        for r=1:size(mapping,1)
            ...
            for d=1:length(allDates)
                ...
                if idx_ROI==0 || isempty(allDates(d).h_calls)
                    ...
                    meanByTrial{d}=allDates(d).meanByTrial_early(:,idx_ROI);
...
%%
    ...
    function MultiDayROIs=make_multiDayROIs_struct_tones(allDates,mapping)
        ...
        numTones=arrayfun(@(x)length(x.tones),allDates); % check if the same # tones played each day
       
        if length(unique(numTones))>1 % if diff number of tones played each day, make index to select only tones that were played every day
            [~,tonesUse]=min(numTones);
            tones=allDates(tonesUse).tones;
            for d=1:length(allDates)
                tonesIdxTmp=zeros(size(tones));
                for t=1:length(tones)
                tonesIdxTmp(t)=find(allDates(d).tones==tones(t),1);
                end
                tonesIdx{d}=tonesIdxTmp;
            end
        else
            tones=allDates(1).tones;
            tonesIdx=repmat({1:length(tones)},length(allDates),1);
        end
            ...
            for d=1:length(allDates)
                ...
                elseif sum(sum(isnan(allDates(d).meanResponsesTone{idx_ROI})))>0
                    ...
                    meanByTrialtmp=cellfun(@(x)mean(x(:,31:91),2),dFbyTone,'UniformOutput',false);
                        ...
                        meanRTones(:,d)=tmpMeanRTones(tonesIdx{d});
                        semRTones(:,d)=tmpSemTones(tonesIdx{d});
                        meanByTrial{d}=meanByTrialtmp(tonesIdx{d});
...
%         timeplot_2sec=((1:size(allDates_calls_2sec(1).dFByTrial{1},2))-1*30)/30;
