function [ex_image,reg_metrics_g,reg_metrics_r]=registration_res_multichannel(varargin)
...
ex_image=ref;
imagesc(ref);
reg_metrics_g=cell(length(stackNames),1);
reg_metrics_r=reg_metrics_g;
fft2Ref=fft2(ref);
for K=1:length(stackNames);
    ...
    elseif numChan==2
        ...
        dft_grn=zeros(size(curr_stack_grn,3),4);
        dft_red=dft_grn;
        parfor i=1:size(curr_stack_grn,3)
                        
            curr_im_grn=curr_stack_grn(:,:,i);
            
            [dft_out,reg]=dftregistration(fft2Ref,fft2(curr_im_grn),100);
        ...
        parfor i=1:size(curr_stack_red,3)
            ...
            [dft_out,reg]=dftregistration(fft2Ref,fft2(curr_im_red),100);
            reg_red=abs(ifft2(reg));
            reg_red_all(:,:,i)=reg_red;
            dft_red(i,:)=dft_out;
        end
        reg_metrics_g{K}=dft_grn;
        reg_metrics_r{K}=dft_red;
    ...
    save([dir_to_save,'reg_metrics.mat'],'reg_metrics_g','reg_metrics_r');
end
